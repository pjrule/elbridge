...
class Graph:
    ...
    """
    def __init__(self, gdf: GeoDataFrame, fusions: List[Callable],
                 n_districts: int, contiguity: str, pop_col: str,
                 city_col: str, county_col: str, pop_tolerance: float):
        """
    ...
    def update(self, vtds: List[int]) -> bool:
        ...
        enclosed_vtds = self.graph.validated(vtds)
        if enclosed_vtds:
    ...
    def _build_graph(self):
        ...
        self.graph = CGraph(self.adj)

    def _init_state(self) -> Dict:
        """ Constructs the initial state dictionary of the graph. """
        vtd_by_district = list(self.df.index) + ([] * (self.n_districts - 1))
        return {
            'done': False,
            'pop_debt': 0,
            'district': 1,
    ...
    def _next_dist_state(self) -> Dict:
        ...
        validity.py#L75
        """
        lower_tol = self.pop_target * (1 - self.pop_target)
        upper_tol = self.pop_target * (1 + self.pop_target)
        # TODO is this safe?
    ...
    def reset(self, district_idx: int):
        """
        ...
        self.states = self.states[:district_idx]
        self.dist_state = self._next_dist_state()
    ...
    def to_dict(self) -> Dict[int, int]:
        ...
        """
        transformed = {}
        for district_idx, vtds in self.state['vtds_by_district']:
            for vtd in vtds:
    ...
    def border_vtds(self, vtds: List[int]) -> List[int]:
        """
        Returns the indices of the VTDs bordering ``vtds``. 
...
def _vtd_indices(gdf: GeoDataFrame, div_col: str) \
    ...
    """
    vtd_to_div = {}
    vtds_in_div = defaultdict(list)
    for vtd_idx, vtd in gdf.iterrows():
        vtd_to_div[vtd_idx] = vtd[div_col]
        vtds_in_div[div_col].append(vtd_idx)
