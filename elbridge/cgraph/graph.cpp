...
namespace graph {
    ...
    bool Graph::shared_border(std::vector<int> unallocated, bool any){
        /*
         * Given an array of VTD IDs, determine whether at least one of the
         * VTDs is contiguous to the current (or prev) district.
         */
        ...
        for(int vtd_idx: unallocated){
            ...
            if(!on_border[district]){
                for(auto neighbor_vtd: vtd.neighbors){
                    bool in_unallocated = std::binary_search(unallocated.begin(), unallocated.end(),
                                                             neighbor_vtd->get_id());
    ...
    bool Graph::contiguous(std::vector<int> unallocated){
        ...
        Given an array of VTD IDs, determine whether:
            ...
            2. either:
                ...
                (b) at least one of the VTDs if contiguous to the previous district
                    (if current district has no VTDs)
        ...
        if(unallocated.size() > 1){
            ...
            while(!to_visit.empty()){
                ...
                for(auto n: vtds.at(vtd).neighbors){
                    bool vtd_in_unallocated = std::binary_search(unallocated.begin(),
                                                                 unallocated.end(), n->get_id());
                    if(n->get_district() == NO_DISTRICT && !visited[n->get_id()] &&
                       vtd_in_unallocated){
    ...
    std::vector<int> Graph::validate(std::vector<int> unallocated){
        /*
         * Ensure that a a new allocation doesn't create two or more 
         * pockets of whitespace. 
         */
        // DFS based on BFS 
        ...
        for(auto v: vtds){
            bool vtd_in_unallocated = std::binary_search(unallocated.begin(),
                                      unallocated.end(), v.second.get_id());
            if(v.second.get_district() == NO_DISTRICT && !vtd_in_unallocated &&
               v.second.neighbors.size() > 0){
        ...
        while(!to_visit.empty()){
            ...
            for(auto n: vtds.at(id).neighbors){
                bool vtd_in_unallocated = std::binary_search(unallocated.begin(),
                                          unallocated.end(), n->get_id());
                if(n->get_district() == NO_DISTRICT && !discovered[n->get_id()]
                   && !vtd_in_unallocated){
                    to_visit.push(n->get_id());
        ...
        if(ws_visited > (ws_left - ws_visited - unallocated.size())){
            ...
            for(auto a: vtds){
                if(a.second.get_district() == NO_DISTRICT && !discovered[a.second.get_id()]){
                    bool vtd_in_unallocated = std::binary_search(unallocated.begin(),
                                              unallocated.end(), a.second.get_id());
